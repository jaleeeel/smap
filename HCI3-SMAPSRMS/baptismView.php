...
error_log("baptismView.php accessed with id: " . ($_GET['id'] ?? 'none'));

if (!isset($_GET['id']) || !is_numeric($_GET['id']) || intval($_GET['id']) <= 0) {
    echo "Invalid request: Missing or invalid ID parameter.";
    error_log("Invalid request in baptismView.php: id param missing or invalid.");
    exit;
}
$id = intval($_GET['id']);
...
if (!$record) {
    ...
    error_log("Record not found in baptismView.php for id: " . $id);
    exit;
}
?>
    ...
    <div class="card">
        ...
        <div class="record-row">
            ...
            <div class="record-value">
                <?php
                // Check if godparents is a JSON array string and decode it, else display as is
        ...
        <div class="record-row">
            <div class="record-label"><i class="fas fa-cross"></i>Baptism Date:</div>
            <div class="record-value"><?= date('m/d/Y', strtotime($record['baptism_date'])) ?></div>
        </div>
        <div class="record-row">
            <div class="record-label"><i class="fas fa-clock"></i>Time:</div>
            <div class="record-value"><?= htmlspecialchars($record['baptism_time']) ?></div>
        </div>
        <div class="record-row">
        ...
        <a href="baptismalRec.php" class="btn-back"><i class="fas fa-arrow-left"></i> Back to Records</a>
    </div>
</body>
</html>
