...
$stmt = $pdo->prepare("SELECT * FROM communion_records WHERE id = ?");
$stmt->execute([$id]);
$record = $stmt->fetch();
...
<body>
    <button class="btn btn-secondary go-back-btn no-print" onclick="window.location.href='communion.php'" style="position: fixed; top: 15px; left: 15px; z-index: 1050; background-color: green; border-color: gray; color: white;">Go Back</button>
    ...
    <div class="certificate-container shadow">
        ...
        <div class="certificate-title">Certificate of Communion</div>
        ...
        <div class="content">
            <p>that <strong><?= htmlspecialchars($record['participant_name']) ?></strong>, born on <?= date('F j, Y', strtotime($record['dob'])) ?>, has received the Holy Communion on <br><?= date('F j, Y', strtotime($record['communion_date'])) ?> by <?= htmlspecialchars($record['priest_name']) ?>. Student in <?= htmlspecialchars($record['school_grade']) ?>.</p>
    
            <p>child of <?= htmlspecialchars($record['guardian_name']) ?>, who lives in <?= htmlspecialchars($record['address']) ?></p>
